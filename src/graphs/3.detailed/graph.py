...
class GraphType(Enum):
    DIRECTED = "directed"      # MISTAKE: I used ":" instead of "="
    UNDIRECTED = "undirected"  # MISTAKE: i typed "," after each enum value


class NodeNameType(Enum):
    INTEGER = "integer"
    STRING = "string"
...
class Transformer:
    """
        Transformer class to transform string node names to indice node names and vice versa.
    """
    def __init__(self):
        self.transformer: dict[str, int] = {}
        self.reverse_transformer: dict[int, str | None] = {-1: None}

    def add(self, word: str) -> int:
        if word in self.transformer:
            return self.transformer[word]
        word_index = len(self.transformer)
        self.transformer[word] = word_index
        self.reverse_transformer[word_index] = word
        return word_index

    def transform(self, word: str) -> int:
        return self.transformer.get(word, -1)

    def inverse(self, index: int) -> str:
        return self.reverse_transformer.get(index, '')


class BaseGraph:

    def __init__(self, graph_type: GraphType, node_name_type: NodeNameType):
        self.graph_type: GraphType = graph_type
        self.node_name_type: NodeNameType = node_name_type
    ...
    def build_graph_from_edges(self, V: int, edges: list[tuple[str | int, str | int, float]]):
        """
            Builds adjacency list representation from a list of edges.
        ...
        for source, destination, weight in edges:
            if self.node_name_type == NodeNameType.STRING:
                source = self._transformer.add(source)
                destination = self._transformer.add(destination)
    ...
    def build_graph_from_matrix(self, adjacency_matrix: list[list[float]]):
        """
            Builds adjacency list representation from a 2D matrix.

            Args:
                adjacency_matrix: 2D matrix where adjacency_matrix[i][j] represents edge weight from node i to node j
    ...
    def bfs(self):
        ...
        parent: list[int | str | None] = [-1] * V  
        connected_components: list[list[int | str]] = []  
        ...
        # Transform node names back to original strings if needed
        if self.node_name_type == NodeNameType.STRING:
            for i in range(len(parent)):
                parent[i] = self._transformer.inverse(parent[i])
            for component in connected_components:
                for i in range(len(component)):
                    component[i] = self._transformer.inverse(component[i])
    ...
    def sssp_bfs(self, source_node: int | str, destination_node: int | str) -> tuple[list[int | str], int] | None:
        """
        ...
        if self.node_name_type == NodeNameType.STRING:
            source_node = self._transformer.transform(source_node)
            destination_node = self._transformer.transform(destination_node)
        ...
        shortest_path: list[int | str] = self._reconstruct_path(source_node, destination_node, parent)
        ...
        if self.node_name_type == NodeNameType.STRING:
            for i in range(len(shortest_path)):
                shortest_path[i] = self._transformer.inverse(shortest_path[i])
    ...
    def sssp_dijkstra(self, source_node: int | str, destination_node: int | str) -> tuple[list[int | str], float] | None:
        """
    ...
    def floyd_warshall(self, adjacency_matrix: list[list[float]]):
        ...
        if self.graph_type == GraphType.UNDIRECTED:
            for source_idx in range(V): # O(V^2 / 2) TC
                for destination_idx in range(source_idx + 1, V):
...
class DirectedGraph(BaseGraph):
    ...
    def __init__(self, node_name_type: NodeNameType = NodeNameType.INTEGER):
        super().__init__(GraphType.DIRECTED, node_name_type)

    # ********* Node & Edge Operations *********
    def get_out_degree(self, node: int | str):
        """
        ...
        """
        if self.node_name_type == NodeNameType.STRING:
            node = self._transformer.transform(node)
        return len(self._adjacency_list[node])

    def get_in_degree(self, node: int | str):
        """
        ...
        if len(topological_order) == V:
            if self.node_name_type == NodeNameType.STRING:
                for i in range(len(topological_order)):
                    topological_order[i] = self._transformer.inverse(topological_order[i])
        ...
        if self.node_name_type == NodeNameType.STRING:
            for i in range(len(strongly_connected_components)):
                for j in range(len(strongly_connected_components[i])):
                    strongly_connected_components[i][j] = self._transformer.inverse(strongly_connected_components[i][j])
        return strongly_connected_components

    # RARELY ASKED ********* SSSP Methods *********
    def bellman_ford(self, source_node: int | str, destination_node: int | str):
        """
...
class DisjointSet:
    ...
    def find_ultimate_parent(self, node: int | str):
        """
...
class UndirectedGraph(BaseGraph):
    ...
    def __init__(self, node_name_type: NodeNameType = NodeNameType.INTEGER):
        super().__init__(GraphType.UNDIRECTED, node_name_type)

    # ********* Node & Edge Operations *********
    def get_degree(self, node: int | str):
        """
    ...
    def mst_prim_s_algorithm(self):
        ...
        mst_edges: list[tuple[float, int | str, int | str]] = []
        ...
        if self.node_name_type == NodeNameType.STRING:
            for iter, (weight, source, destination) in enumerate(mst_edges):
                mst_edges[iter] = (weight, self._transformer.inverse(source), self._transformer.inverse(destination))
    ...
    def mst_kruskal_s_algorithm(self):
        ...
        sorted_edges: list[tuple[float, int | str, int | str]] = sorted(
            [
    ...
    def get_bridges(self):
        ...
        V = len(self._adjacency_list)
        visited: list[bool] = [False] * V
        bridges: list[tuple[float, int | str, int | str]] = []
        ...
        if self.node_name_type == NodeNameType.STRING:
            for iter, (weight, source, destination) in enumerate(bridges):
                bridges[iter] = (weight, self._transformer.inverse(source), self._transformer.inverse(destination))
    ...
    def get_articulation_points(self):
        ...
        if self.node_name_type == NodeNameType.STRING:
            return sorted([self._transformer.inverse(node) for node in articulation_points])
